...
def run(question: str, max_steps: int = 10, verbose: bool = True, chat_history=None) -> Generator[dict, None, None]:
    """
    ...
    if chat_history and len(chat_history) > 0:
        for user_q, bot_ans in chat_history:
            ...
            messages.append({"role": "assistant", "content": f"Final Answer: {bot_ans}"})
    # 再追加当前最新用户问题
...
def run_and_print(question: str, max_steps: int = 10, chat_history=None):
    ...
    for step_data in run(question, max_steps=max_steps, chat_history=chat_history):
        ...
        if stype == "action":
            ...
            final_answer = step_data["answer"]  # 保存最终回答
            print(f"\n{'─'*60}")
