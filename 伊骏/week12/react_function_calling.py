...
def run(question: str, max_steps: int = 10, chat_history=None) -> Generator[dict, None, None]:
    """
    ...
    if chat_history and len(chat_history) > 0:
        for user_q, bot_ans in chat_history:
            messages.append({"role": "user", "content": user_q})
            messages.append({"role": "assistant", "content": bot_ans})
    # 追加当前最新用户提问
...
def run_and_print(question: str, max_steps: int = 10, chat_history=None):
    print(f"\n{'='*60}")
    print(f"问题: {question}")
    ...
    final_answer = ""  # 新增：保存本轮最终回答

    for step_data in run(question, max_steps=max_steps, chat_history=chat_history):
        ...
        if stype == "action":
            ...
            final_answer = step_data["answer"] # 记录最终答案
            print(f"\n{'─'*60}")
            print(_c("final", f"\n✅ Final Answer:\n{step_data['answer']}"))
