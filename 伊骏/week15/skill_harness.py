...
class SkillHarness:
    ...
    def execute(self, skill_id: str, inputs: Dict[str, Any], session_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        :param inputs: 调用入参
        :param session_context: 当前会话上下文（session、memory、全局资源）
        :return: 标准化执行结果
        """
        try:
            ...
            # ========== 分支1：存在内嵌Python代码块，动态执行Code ==========
            if skill.code_block and skill.code_block.strip():
                # 注入运行时内置变量，给skill代码读取
                exec_globals = {
                    "__builtins__": __builtins__,
                    "skill_inputs": inputs,
                    "skill_session_ctx": session_context
                }
                exec_locals = {}
                # 执行skill内python代码（仅定义run函数，不会自动运行）
                exec(skill.code_block, exec_globals, exec_locals)
                # 修复点1：必须校验存在run入口函数并手动调用
                if "run" not in exec_locals:
                    raise RuntimeError("Skill Code块必须定义 run(params, ctx) 入口函数")
                # 手动调用run，把返回结果存入return_value
                exec_locals["return_value"] = exec_locals["run"](inputs, session_context)
                skill_result = exec_locals["return_value"]
                # 校验返回类型
                if not isinstance(skill_result, dict):
                    raise RuntimeError("Skill Code return 必须返回字典格式")
                return skill_result
            # ========== 分支2：无代码块，纯Prompt技能（原有逻辑保留） ==========
            result_data = {
                "skill_id": skill_id,
                "status": "success",
