...
#
# from Homework.week11.function_call_mcp_cli.src.rag_backend import BASE_DIR
...
def main():
    ...
    # ========== 新增：初始化渐进式加载Harness ==========
    # 拼接skills文件夹路径
    BASE_DIR = Path(__file__).parent.parent  # → agent_memory_system
    # 向上跳出一层，week13/skills
    SKILLS_ROOT = BASE_DIR.parent / "skills"
    harness = SkillHarness(str(SKILLS_ROOT))
    # 打印注册日志，验证启动仅加载元数据（作业渐进特性证明）
    ...
    while True:
        ...
        if user_input.startswith("/skill"):
            # 格式：/skill calculator a=10 b=20 op=+
        ...
        # ========== 在这里插入 Skill 识别判断逻辑 ==========
        # 1. 构造工具判断prompt，让LLM输出是否调用技能、参数
        judge_prompt = f"""
        ...
        【重要技能参数约束】
        calculator（四则计算器）固定3个必填参数：
        inputs = {{"a": 第一个数字, "b": 第二个数字, "op": 运算符}}
        运算符op可选值："+", "-", "*", "/"
        示例：128乘以7 → op="*"
        ...
        输出示例（计算器参考）：
        {{"call_skill": true, "skill_id": "calculator", "inputs": {{"a":128,"b":7,"op":"*"}}}}

        硬性要求：
        1. inputs 里的key名称必须严格匹配技能规定参数名(a,b,op)，不能自定义名称；
        2. 只输出纯净JSON字符串，不要```json标记、不要注释、不要多余中文；
        3. JSON布尔值使用小写 true / false，不要使用 True / False；
        4. 遇到数学运算**必须调用calculator工具，禁止自己直接计算结果**。
        """
        ...
        if judge_data.get("call_skill", False) is True:
            ...
            # # 将技能执行结果转为上下文文本，拼入system prompt
            # skill_output_content = f"\n## 工具执行结果\n{json.dumps(skill_res, ensure_ascii=False, indent=2)}"
            # 工具结果前置强制指令，强制模型采信工具数值
            skill_output_content = """
            ## 【不可违反强制指令】
            下方计算器工具输出的数字是100%准确官方结果，你绝对不能自己计算、估算、改动数字；
            回答用户时直接复用工具给出的数值，严格使用纯文本，不允许**加粗、星号、任何格式符号。
            ## 工具执行结果
            """ + json.dumps(skill_res, ensure_ascii=False, indent=2)
        # ==================================================
